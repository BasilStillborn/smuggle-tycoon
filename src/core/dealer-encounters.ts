...
const DEALER_TEMPLATES: EncounterTemplate[] = [
  ...
  },
  {
    ...
    generate: (player: PlayerState, country: Country, dealer: DealerProfile, dealContext?: DealContext): ChoiceEvent => {
      ...
      context: `Your contact gestures Angelo to a corner table, nursing a drink. ${p({ gender: 'male' }).He} slides a newspaper across. "It's inside," ${p({ gender: 'male' }).he} mutters. "Price went up ${Math.round(pct * 100)}%. Supply chain issues."`,
      choices: [
        { id: 'accept_hike', text: `Accept the new price (+$${increase}). You're already here.`, odds: 0.70 + player.credibility * 0.002, successEffects: { cashDelta: -increase, heatDelta: 5, reputationDelta: 0, credibilityDelta: 3, inventoryLost: false, message: `${p({ gender: 'male' }).He} folds the paper. "Smart man." The deal is done. You overpaid — but you have the product.` }, failEffects: { cashDelta: -increase, heatDelta: 10, reputationDelta: 0, credibilityDelta: -5, inventoryLost: true, message: `${p({ gender: 'male' }).He} takes your money, stands, and walks out. "Wrong move, Angelo, you nonce." A waiter bumps into you — when you look down, the newspaper is gone. So is the package inside it.` } },
        { id: 'negotiate', text: 'Push back. We agreed on a price.', odds: 0.30 + player.credibility * 0.005, successEffects: { cashDelta: 0, heatDelta: 5, reputationDelta: 3, credibilityDelta: 10, inventoryLost: false, message: `${p({ gender: 'male' }).He} studies Angelo, then laughs — a short, surprised bark. "Alright. You've got stones." ${p({ gender: 'male' }).He} honours the original price. Respect earned.` }, failEffects: { cashDelta: 0, heatDelta: 15, reputationDelta: 0, credibilityDelta: -10, inventoryLost: false, message: `${p({ gender: 'male' }).He} shrugs and closes the paper. "No deal, then." ${p({ gender: 'male' }).He} leaves you sitting alone at the table.` } },
  ...
  },
  {
    ...
    generate: (player: PlayerState, country: Country, dealer: DealerProfile, dealContext?: DealContext): ChoiceEvent => {
      ...
      choices: [
        { id: 'pay_tax', text: `Pay the "tax" — $${tax}. Keep it civil.`, odds: 0.55 + oddsBoost + player.credibility * 0.002, successEffects: { cashDelta: -tax, heatDelta: 5, reputationDelta: 0, credibilityDelta: 2, inventoryLost: false, message: `${p({ gender: 'male' }).He} pockets the cash. "Pleasure doing business, Angelo." His men part like a curtain. You pass unharmed.` }, failEffects: { cashDelta: -tax, heatDelta: 20, reputationDelta: 0, credibilityDelta: -10, inventoryLost: true, message: `${p({ gender: 'male' }).He} takes the money — then nods. His men grab you from behind. They take everything. "Tax went up," ${p({ gender: 'male' }).he} says, walking away.` } },
        { id: 'stand_ground', text: 'Stand your ground. You don\'t pay tax to street trash.', odds: 0.25 + player.notoriety * 0.004, successEffects: { cashDelta: 0, heatDelta: 10, reputationDelta: 5, credibilityDelta: 15, inventoryLost: false, message: `Something in Angelo's eyes makes him pause. ${p({ gender: 'male' }).He} laughs — a genuine one this time. "Tough bastard. Alright. Pass." The men part. You walk through without paying a cent.` }, failEffects: { cashDelta: 0, heatDelta: 30, reputationDelta: 0, credibilityDelta: -20, inventoryLost: true, message: 'They jump Angelo. "Wrong move, you retard." When you come to, your pockets are empty and your face is in the gutter. Pride wounded. Wallet destroyed.' } },
  ...
  // === TIER 2: Medium credibility (25-60) ===
  {
    ...
    generate: (player: PlayerState, country: Country, dealer: DealerProfile): ChoiceEvent => ({
      ...
      context: `Angelo meets his contact in a dimly lit bar. ${p({ gender: 'male' }).He} slides a sample across the worn wooden table. "Top shelf. Fresh from the lab. My guy wants to move double the usual volume. You interested?"`,
      choices: [
  ...
  },
  {
    ...
    generate: (player: PlayerState, country: Country, dealer: DealerProfile): ChoiceEvent => ({
      ...
      context: `Your contact seems unusually happy to see Angelo. "Got some good news for you," ${p({ gender: 'male' }).he} grins. "Supplier overproduced. I can give you an extra fifty percent at half price. Take it now, or it goes to someone else tonight."`,
      choices: [
        ...
        { id: 'refuse_bonus', text: 'Stick with the original deal. No surprises.', odds: 0.85, successEffects: { cashDelta: 0, heatDelta: 5, reputationDelta: 0, credibilityDelta: 3, inventoryLost: false, message: `${p({ gender: 'male' }).He} shrugs. "Your loss, my friend." The original deal goes through cleanly. Slow and steady.` }, failEffects: { cashDelta: 0, heatDelta: 5, reputationDelta: 0, credibilityDelta: 0, inventoryLost: false, message: `${p({ gender: 'male' }).He} looks disappointed but proceeds. The deal is fine.` } },
      ],
    }),
  },
